...
The tutorial recommends [Solidworks Exporter](http://wiki.ros.org/sw_urdf_exporter) for <inertial> properties. However I have designed in FreeCAD. 

I found several methods to obtain inertial properties from FreeCAD models:
...
If you've found out the center of mass experimentally, fill in this  value, otherwise fill in the correctly scaled value computed from the model. *The rotational elements can define a different coordinate for the moment of inertia axes.* (Meaning??)
...
- [ ] write a python script to read from this CSV file the center of mass coordinates, volume, moment of inertia etc and generate the <inertial> tag. Note: careful, the CSV keeps all the saves one after another!!
